# Autonomous Mode v3.2 - 범용 프레임워크

> **`/autonomous [작업]` 하나로 모든 최적화가 자동 적용됩니다.**
>
> v3.2: Agent Teams Phase 4.5 강화 - 매 작업마다 팀원 필요 여부 필수 판단 + 결과 출력 의무화.
...
| SSOT 파일 수정 필요 | 다관점 분석 (리뷰, 조사) |
| 독립 파일 2개 이하 | 독립 기능 동시 개발 (프론트A + 백엔드B) |
| 동일 패턴 반복 (조율 비용 > 이득) | 버그 원인 경쟁 조사 |
| 단순 질문/조사/문서 수정 | 대규모 리팩토링 (영역별 분리 가능) |
...
**교훈**: 에이전트가 보고한 값이 실제와 다른 사례가 반복 발생.
