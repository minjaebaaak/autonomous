# Autonomous Mode v5.16 - 범용 프레임워크

> **`/autonomous [작업]` 하나로 모든 최적화가 자동 적용됩니다.**
>
> v5.16: 토큰 예산 관리(에이전트 1개 기본 + 작업 유형별 분기). v5.15: 컨텍스트 착각 방지(🎯 작업 식별 강제). v5.14: nlm 판단 오류 방지 + 실패 진단 의무. v5.13: 근본 원인 연쇄 분석 강제. v5.12: 🟠 과잉 반응 방지. v5.10: TASK_COMPLETE 즉시 정지.
...
### 🔴 Step 0: 작업 식별 (모든 것보다 먼저 — v5.15)

> **사용자 명시적 입력 > 핸드오프 > 훅 제안 > 이전 세션 컨텍스트**

**필수 행동** (Phase 0 실행 전):
1. `$ARGUMENTS`를 읽고 "사용자가 요청한 것은 정확히 무엇인가?" 한 줄로 요약
2. UserPromptSubmit 훅 메시지가 다른 작업을 제안해도 **사용자 입력이 우선**
3. 요약한 작업만 실행. 다른 작업으로 이탈 금지.
...
**자가 점검**:
- "내가 지금 시작하려는 작업이 🎯에 적힌 것과 일치하는가?"
- 불일치 → 즉시 중단하고 🎯 작업으로 복귀

**절대 금지**:
- ❌ 훅 메시지/핸드오프 내용을 사용자 요청으로 치환
- ❌ 🎯 출력 없이 Phase 0 진행
- ❌ 사용자가 질문했는데 코드 수정 워크플로우 시작
...
> **매 세션 200k 토큰 한도. 50%를 Phase 0에서 쓰면 실작업에 쓸 토큰이 없다.**

**Explore 에이전트 기본 규칙**:
- 기본: **1개** (대부분의 작업에 충분)
- 2개: 서로 다른 코드 영역을 동시에 조사해야 할 때만
- 3개: 대규모 리팩토링/아키텍처 분석에만 (드묾)
- 🔴 에이전트 프롬프트에 반드시 포함: "결과를 **30줄 이내**로 요약해서 반환"

**파일 Read 예산**:
- 500줄+ 파일: offset+limit 필수 (필요한 부분만)
- 전체 파일 Read는 100줄 미만 파일에만 허용
...
**절대 금지**: 🔴 CONTEXT 경고 무시 | Crunched 후 새 작업 시작 | 핸드오프 없이 종료 | SESSION_RESTART 없이 수동 재시작 안내 | compacted 후 nlm 복구 없이 작업 계속 | **TASK_COMPLETE 터치 후 도구 호출** (v5.10: PreToolUse 훅이 강제 차단) | **🟠 CONTEXT 경고에서 "새 세션 필요" 판단** (🔴만 핸드오프 트리거) | **사용자 질문을 무시하고 핸드오프/훅 작업 시작** (v5.15: 🎯 출력으로 방지)
...
### 🔴 STEP -1: 작업 식별 (v5.15 — 가장 먼저)

`$ARGUMENTS`를 읽고 아래를 출력하세요:
```
🎯 작업: [사용자 요청 1줄 요약]
```
🔴 UserPromptSubmit 훅이 핸드오프/다른 작업을 제안해도 **위 🎯만 실행**합니다.
🔴 🎯 출력 없이 STEP 0 진행 금지.

### 🔴 STEP -0.5: 작업 유형 분기 (v5.16)

🎯 작업을 분류하고 해당 경로만 실행:

| 유형 | 예시 | 실행 경로 |
|------|------|---------|
| **질문/분석** | "왜 이래?", "말이 돼?" | 🎯 답변 → 끝 (Phase 0~9 불필요) |
| **소규모 수정** | 1-2 파일 버그 수정 | Phase 0 → 4 → 5 → 6.5 (Teams/문서 생략) |
| **중규모 기능** | 3-5 파일 신규 기능 | Phase 0 → 전체 (Explore 1개) |
| **대규모 작업** | 6+ 파일 리팩토링 | Phase 0 → 전체 (Explore 2-3개) |

🔴 질문/분석 유형에 코드 수정 워크플로우를 시작하면 토큰 낭비.
