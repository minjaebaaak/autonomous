# Autonomous Mode v5.24 - 범용 프레임워크

> **`/autonomous [작업]` 하나로 모든 최적화가 자동 적용됩니다.**
>
> v5.24: `/autonomous`만 호출 시 핸드오프 우선 확인. v5.23: session ID 기반 완전 격리. v5.20: scarcity 제거. v5.16: 작업 유형 분기. v5.15: 🎯 작업 식별. v5.14: 실패 진단 의무.
...
- 필요 시 grep으로 교차 검증한다
- **🔴 nlm 성공 시 Read 도구로 기술문서 전체를 읽지 않는다**
...
> nlm/repomix/grep이 Read보다 빠르고 정확하다. 아래 표에 따라 도구를 선택한다.
...
**Read 권장 용도**: Edit 직전 offset+limit, 특수 파일(설정/JSON), 수정 검증(소형).

### 효율적 탐색 (v5.20)

**Explore 에이전트**:
- 기본: **1개**. 2개: 다른 코드 영역 동시 조사. 3개: 대규모 리팩토링.
- 에이전트 프롬프트에 "결과를 **30줄 이내**로 요약" 포함.

**파일 Read**:
- 500줄+ 파일: offset+limit 필수
- 전체 파일 Read는 100줄 미만에만 허용

**Phase별 출력**:
- 단순 작업: Phase 4.5 팀원 판단 = "단독 진행" 1줄
- 질문/분석: Phase 2~9 건너뛰고 바로 답변
...
**A) `$ARGUMENTS`가 있으면** → 🎯 출력 후 STEP -0.5로:
```
🎯 작업: [사용자 요청 1줄 요약]
```

**B) `$ARGUMENTS`가 비어있으면** → **핸드오프부터 확인**:
1. `~/.claude/state/handoffs/` 에서 현재 프로젝트 핸드오프 검색
2. 유효 핸드오프 있으면 → 읽고 복원 → 해당 작업 이어서 진행
3. 없으면 → "어떤 작업을 수행할까요?" 질문

🔴 `$ARGUMENTS` 비어있을 때 핸드오프 확인 없이 "작업을 입력해주세요" 출력 금지.
...
🔴 질문/분석 유형에 코드 수정 워크플로우를 시작하지 않는다.
