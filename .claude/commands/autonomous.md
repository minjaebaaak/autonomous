# Autonomous Mode v3.3 - 범용 프레임워크

> **`/autonomous [작업]` 하나로 모든 최적화가 자동 적용됩니다.**
>
> v3.3: Agent 결과 교차 검증 강화 + 읽기 전용 Agent 수량 유연화.
...
| 단독 진행 | 수정 팀원 구성 (2-3명) | 조사/분석 에이전트 (상한 없음) |
|----------|-----------------|--------------------------|
| 순차 의존 작업 | 독립 파일 3개+, SSOT 미포함 | 전수 조사 지시 시 파일당 1명 |
| SSOT 파일 수정 필요 | 다관점 분석 (리뷰) | 대상 파일 5~10개: 2~3명 |
| 독립 파일 2개 이하 | 독립 기능 동시 개발 | 대상 파일 10개+: 3~5명 |
| 동일 패턴 반복 | 버그 원인 경쟁 조사 | 대형 파일(3000줄+): 개별 배정 |
| 단순 질문/문서 수정 | 대규모 리팩토링 | 읽기 전용 → 파일 충돌 0 |

**핵심**: Explore/Plan 에이전트(읽기 전용)는 파일 수정 불가이므로, 충돌 위험 없이 범위에 비례하여 확장 가능. 기존 3명 제한은 **수정 작업에만** 적용.
