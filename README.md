...
| NAVIFACT | `projects/navifact/` | 팩트 기반 역사적 사건 추적 포털 — Next.js 15 + FastAPI + Neo4j |
...
### v5.24 (2026-03-23)
`/autonomous`만 호출 시 핸드오프 우선 확인: $ARGUMENTS 비어있으면 핸드오프부터 검색. "작업을 입력해주세요" 대신 이전 작업 자동 복원. fanagement에서 발생한 핸드오프 미감지 문제 해결.
...
### v5.20 (2026-03-23)
Scarcity 프레이밍 근본 제거: "컨텍스트 절약 규칙" → "도구 선택 가이드", "토큰 예산 관리" → "효율적 탐색", "한도/부족/소진/예산/낭비/절약" 단어 전량 삭제. "기억하지 말고 기록하라" 프레이밍 제거. 핸드오프 메시지에서 "컨텍스트 소진" 제거. 3개 프로젝트(sharemanager/fight/fanagement)에서 반복 발생한 불필요한 세션 전환 근본 원인 해결.

### v5.19 (2026-03-19)
Stop 훅 자동 핸드오프: 세션 비정상 종료 시 session-report.sh가 자동으로 핸드오프 노트 생성 (TASK_COMPLETE 있으면 생략). session ID 격리: 핸드오프에 session 필드 추가, Step 1.6에서 자기 세션 핸드오프 건너뛰기. 5분 이내 중복 방지.

### v5.18 (2026-03-19)
컨텍스트 트리거 규칙 분리: "언제 핸드오프" (트리거) = 훅 전용, "어떻게 핸드오프" (절차) = autonomous.md. 세션 전환 전략 v5.7 삭제 ("항상 낫다" 무조건적 선언 제거). v5.12 🟠/🔴 구분 삭제. v5.17 "훅 전용" 패치 삭제 (근본 수정으로 대체). token-optimizer.sh 🔴 메시지에 행동 지침 인라인.

### v5.17 (2026-03-18)
컨텍스트 판단 훅 전용: Claude 자체의 "컨텍스트 부족" 추측 금지. 핸드오프는 오직 token-optimizer.sh 🔴 경고 시에만. 훅 경고 없이 "컨텍스트 제한적" 판단 → 절대 금지 목록 추가.

### v5.16 (2026-03-17)
토큰 예산 관리: Explore 에이전트 기본 1개(3→1), 에이전트 프롬프트에 "30줄 이내 요약" 필수, 500줄+ 파일 offset+limit 강제. 작업 유형별 분기(STEP -0.5): 질문/분석은 Phase 0~9 건너뛰고 바로 답변, 소규모 수정은 축약 경로.
...
### v5.5.1 (2026-02-24)
세션 종료 UX 개선: 컨텍스트 소진 시 "새 세션에서 진행하세요" 대신 "`/clear` → `/autonomous`" 안내 출력. 같은 tmux 창에서 핸드오프 자동 복원. 절대 금지에 "새 세션 권장" / "추가 작업이 있으시면..." 추가.
...
| 버전 | 날짜 | 변경사항 |
|------|------|----------|
| **v5.14** | **2026-03-12** | **nlm 판단 오류 방지 + 실패 진단 의무**: Step 1을 CLAUDE.md 의존에서 `command -v nlm` 기반으로 변경 (compaction 후에도 nlm 사용). Phase 5에 "실패 = 진단 기회" 원칙 추가 — 에러 시 원인 분석 → 수정 → 재시도 필수, "안 되네" 포기 금지 |
| v5.13 | 2026-03-12 | **근본 원인 연쇄 분석 강제**: Phase 4를 3단계 인과 추적(현상→근접→근본)으로 강화. 필수 출력 포맷 + 수정 레벨 판단 + 자가 점검 3문항. 현상만 패치하고 "수정 완료" 선언 금지 |
