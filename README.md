...
>
> AEGIS + Ralph Loop + Phase 0 강제 + 에이전트 결과 검증 + 문서 동기화 + 양방향 동기화 + Agent Teams 필수 판단 + 자동 커밋

`/autonomous [작업]` 하나로 모든 최적화가 자동 적용됩니다.

---

## v3.8: 횡단 관심사 계층별 sweep Phase 5.9 추가

v3.8에서 **Phase 5.9**를 추가했습니다:

- **Phase 5.9**: 횡단 관심사 계층별 sweep 원칙 - "한 계층만 수정 ≠ 전수 조사"
- 타임존/인증/로깅 등 횡단 관심사는 6개 계층(모델→서비스→API→프론트→유틸리티→연관패턴) 전부 확인
- ORM `server_default`는 SQL-level fallback — 런타임 write path가 실제 저장값 결정
- "전수 완료" 선언 전 전 계층 체크리스트 확인 필수

### v3.4~v3.7 변경사항
- **v3.7**: 사용자 여정 일관성 원칙 (Phase 5.8)
- **v3.6**: 작업 완료 시 자동 git commit & push (Phase 6.5)
- **v3.5**: Source-Sink 정합성 (Phase 5.6) + 멱등성 원칙 (Phase 5.7)
- **v3.4**: 에이전트 수량 제한 전면 해제 (작업 범위 비례 자율 결정)
- **v3.3**: Agent 결과 교차 검증 강화 (수량 Grep 대조 필수)
...
| **사용자 여정 일관성** | 단일 함수 정확성 ≠ 시스템 정확성, 연결 페이지 데이터 일치 확인 | ✅ 자동 (v3.7) |
| **횡단 관심사 계층별 sweep** | 타임존/인증/로깅 등 전 계층 체크리스트 + ORM write path 추적 | ✅ 자동 (v3.8) |
...
| **Phase 5.8** | 사용자 여정 일관성 | "계산이 맞다 ≠ 시스템이 맞다" - 연결 페이지 데이터 일치 확인 |
| **Phase 6** | 커밋 전 문서 확인 | 문서 업데이트 없이 커밋 금지 |
| **Phase 6.5** | 자동 커밋 & 푸시 | 작업 완료 시 자동 commit & push (질문 금지) |
...
| 버전 | 날짜 | 변경사항 |
|------|------|----------|
| **v3.8** | **2026-02-19** | **횡단 관심사 계층별 sweep Phase 5.9**: "한 계층만 수정 ≠ 전수 조사" - 6계층 체크리스트 + ORM write path 추적 |
| v3.7 | 2026-02-15 | 사용자 여정 일관성 Phase 5.8: "계산이 맞다 ≠ 시스템이 맞다" - 연결 페이지 데이터 일치 확인 |
| v3.6 | 2026-02-12 | 자동 커밋 Phase 6.5: 작업 완료 시 자동 commit & push, Claude Code 내장 규칙 명시적 override |
| v3.5 | 2026-02-12 | Source-Sink 정합성 (Phase 5.6) + 멱등성 원칙 (Phase 5.7) |
| v3.4 | 2026-02-12 | 에이전트 수량 제한 전면 해제 (작업 범위 비례 자율 결정) |
| v3.3 | 2026-02-12 | Agent 결과 교차 검증 강화: 수량 Grep 대조 필수, 교차 검증 완료 후 플랜 작성 |
